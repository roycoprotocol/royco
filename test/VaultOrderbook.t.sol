...
contract VaultOrderbookTest is Test {
   ...
   function testCreateAPOrder(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       uint256 order1Id =
           orderbook.createAPOrder(address(targetVault), address(0), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
       VaultOrderbook.APOrder memory order1 =
           VaultOrderbook.APOrder(order1Id, address(targetVault), alice, address(0), timeToExpiry, tokensRequested, tokenRatesRequested);
       ...
       uint256 order2Id =
           orderbook.createAPOrder(address(targetVault), address(fundingVault), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
       VaultOrderbook.APOrder memory order2 =
           VaultOrderbook.APOrder(order2Id, address(targetVault), alice, address(fundingVault), timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testCannotCreateExpiredOrder(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       uint256 orderId = orderbook.createAPOrder(address(targetVault), address(0), quantity, block.timestamp, tokensRequested, tokenRatesRequested);
   ...
   function testCannotCreateZeroQuantityOrder(uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       orderbook.createAPOrder(address(targetVault), address(0), 0, timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testMismatchedBaseAsset(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       orderbook.createAPOrder(address(targetVault), address(fundingVault2), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testCannotAllocateExpiredOrder(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       uint256 orderId = orderbook.createAPOrder(address(targetVault), address(0), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
       VaultOrderbook.APOrder memory order =
           VaultOrderbook.APOrder(orderId, address(targetVault), alice, address(0), timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testNotEnoughBaseAssetToAllocate(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       uint256 order1Id = orderbook.createAPOrder(address(targetVault), address(0), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
       fundingVault.deposit(quantity, alice);
       uint256 order2Id = orderbook.createAPOrder(address(targetVault), address(fundingVault), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testCancelOrder(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       ...
       uint256 order2Id =
           orderbook.createAPOrder(address(targetVault2), address(0), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);
       uint256 order3Id =
           orderbook.createAPOrder(address(targetVault3), address(0), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);

       VaultOrderbook.APOrder memory order2 =
           VaultOrderbook.APOrder(order2Id, address(targetVault2), alice, address(0), timeToExpiry, tokensRequested, tokenRatesRequested);
       VaultOrderbook.APOrder memory order3 =
           VaultOrderbook.APOrder(order3Id, address(targetVault3), alice, address(0), timeToExpiry, tokensRequested, tokenRatesRequested);
   ...
   function testAllocateOrderFromVault(uint256 quantity, uint256 timeToExpiry, uint256 tokenRateRequested) public {
       vm.assume(quantity > 0);
       vm.assume(quantity <= type(uint256).max / quantity/2);
       vm.assume(timeToExpiry >= block.timestamp);
       vm.assume(block.timestamp <= timeToExpiry);
       ...
       baseToken.approve(address(targetVault), quantity);
       baseToken.approve(address(fundingVault), quantity);
       ERC20(fundingVault).approve(address(orderbook), quantity);
       ...
       uint256 orderId = orderbook.createAPOrder(address(targetVault), address(fundingVault), quantity, timeToExpiry, tokensRequested, tokenRatesRequested);

       VaultOrderbook.APOrder memory order =
           VaultOrderbook.APOrder(orderId, address(targetVault), alice, address(fundingVault), timeToExpiry, tokensRequested, tokenRatesRequested);
