...
contract RecipeOrderbook is Ownable2Step, ReentrancyGuard {
    ...
    function fillLPOrder(LPOrder calldata order, uint256 fillAmount, address frontendFeeRecipient) external {
    ...
    function cancelLPOrder(LPOrder calldata order) external {
    ...
    function cancelIPOrder(uint256 orderID) external {
        IPOrder storage order = orderIDToIPOrder[orderID];
    ...
    function setProtocolFeeClaimant(address _protocolFeeClaimant) external onlyOwner {
        assembly ("memory-safe") {
            sstore(protocolFeeClaimant.slot, _protocolFeeClaimant)
        }
    }
    ...
    function setProtocolFee(uint256 _protocolFee) external onlyOwner {
        assembly ("memory-safe") {
            sstore(protocolFee.slot, _protocolFee)
        }
    }
    ...
    function setMinimumFrontendFee(uint256 _minimumFrontendFee) external onlyOwner {
        assembly ("memory-safe") {
            sstore(minimumFrontendFee.slot, _minimumFrontendFee)
        }
    }

    /// @notice calculates the hash of an order
    function getOrderHash(LPOrder memory order) internal pure returns (bytes32) {
